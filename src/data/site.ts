...
/*
  Цены индивидуальной экскурсии — те же, что называет ИИ-чат (подтверждены
  владельцем 29.07.2026; базовая «330 €/4 ч» подтверждена повторно 06.08).
  Единственное место, где они вписаны: страницы (метки fill.ts), llms.txt,
  резервная база знаний чата и разметка для поисковиков берут их отсюда.
  Лежат в site.ts, а не в tours.ts: скрипт чата импортирует site.ts и не
  должен тянуть весь tours.json.
*/
/** Базовая экскурсия на автомобиле: цена (€) и длительность (часы). */
export const TOUR_PRICE_BASE = 330;
export const TOUR_HOURS_BASE = 4;
/** Каждый дополнительный час (€). */
export const TOUR_PRICE_HOUR = 60;
/** Микроавтобус до 8 человек: цена за базовую программу (€) и доплата за час. */
export const TOUR_PRICE_MINIBUS = 550;
export const TOUR_PRICE_MINIBUS_HOUR = 100;
/** Спринтер до 17 человек: цена за базовую программу (€) и доплата за час. */
export const TOUR_PRICE_SPRINTER = 750;
export const TOUR_PRICE_SPRINTER_HOUR = 140;
